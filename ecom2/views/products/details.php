...
<div class="max-w-5xl mx-auto space-y-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-10 bg-white p-6 md:p-8 rounded-2xl border border-gray-100 shadow-sm">
        <div class="relative bg-gray-50/50 rounded-2xl overflow-hidden border border-gray-100 p-4 flex items-center justify-center h-80 md:h-[400px]">
            <a href="index.php?controller=wishlist&action=toggle&id=<?= $product['id'] ?>" class="absolute top-4 right-4 bg-white shadow p-3 rounded-full hover:scale-110 transition text-base z-10">
                <?= $isWishlisted ? '❤️' : '🤍'; ?>
            </a>
        ...
        <div class="flex flex-col justify-between">
            ...
            <div class="pt-6 border-t border-gray-100 mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <a href="index.php?controller=cart&action=add&id=<?= $product['id'] ?>" class="w-full text-center bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 px-4 rounded-xl transition shadow-sm text-sm flex items-center justify-center space-x-2">
                    <span>Add to Shop Cart 🛒</span>
                </a>
                <a href="index.php" class="w-full text-center bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold py-3.5 px-4 rounded-xl transition text-sm">Back to Catalog</a>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
        ...
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm space-y-4">
            ...
            <?php if (isset($_SESSION['user_id'])): ?>
                <form action="index.php?controller=review&action=add" method="POST" class="space-y-4">
            ...
            <?php else: ?>
                <div class="bg-gray-50 p-4 rounded-xl border border-gray-100 text-center text-xs text-gray-500">
                    ...
                    <a href="index.php?controller=auth&action=login" class="text-blue-600 font-bold hover:underline">Sign In Here →</a>
                </div>
            <?php endif; ?>
        </div>
